...
zoning_bx = pluto_bx %>%
  ...
  rename(GEOID = CT2010) %>%
  select(-other) %>%
  tract.to.geoid(boro_code = "005", tract_var = "GEOID", data = .)
```
...
zoning_mn = pluto_mn %>%
  ...
  rename(GEOID = CT2010) %>%
  select(-other) %>%
  tract.to.geoid(boro_code = "061", tract_var = "GEOID", data = .)
```
...
zoning_qn = pluto_qn %>%
  ...
  rename(GEOID = CT2010) %>%
  select(-other) %>%
  tract.to.geoid(boro_code = "081", tract_var = "GEOID", data = .)
```
...
zoning_si = pluto_si %>%
  ...
  rename(GEOID = CT2010) %>%
  select(-other) %>%
  tract.to.geoid(boro_code = "085", tract_var = "GEOID", data = .)
```
